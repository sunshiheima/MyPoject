...
function getReactRootElementInContainer(container: any) {
  ...
  if (container.nodeType === DOCUMENT_NODE) {//判断是否为根节点document
    return container.documentElement;//为html节点
  } else {
...
function legacyCreateRootFromDOMContainer(
  ...
  const shouldHydrate =
    forceHydrate || shouldHydrateDueToLegacyHeuristic(container);//服务器渲染，默认为false
  // First clear any existing content.
  if (!shouldHydrate) {
    let warned = false;
    let rootSibling;
    while ((rootSibling = container.lastChild)) {//循环删除所有
      if (__DEV__) {
        if (
          !warned &&
  ...
  const isConcurrent = false;//不起用并发模式
...
function legacyRenderSubtreeIntoContainer(
  ...
  let root: Root = (container._reactRootContainer: any);//首次加载没有_reactRootContainer属性
  if (!root) {
    // Initial mount
